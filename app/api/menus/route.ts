  ...
  try {
      ...
      include: {
        ...
        menuItemAssignments: storeId
          ? {
              ...
              include: {
                priceOverrides: {  // FIXED: Correct relation name
                  where: {
                    OR: [
                      { expiresAt: null },  // FIXED: camelCase field name
                      { expiresAt: { gte: new Date() } },
                    ],
                  },
                  orderBy: { effectiveFrom: "desc" },  // FIXED: camelCase field name
                  take: 1,
                },
              },
            }
    ...
    if (storeId) {
      formattedMenus = menuTemplates.map((template) => {
        const assignment = template.menuItemAssignments?.[0] as any;  // Type assertion needed for conditional includes
        const priceOverride = assignment?.priceOverrides?.[0];
        ...
        return {
          ...
          displayOrder: assignment?.display_order ?? 99,  // FIXED: snake_case field in Prisma
          overriddenPrice: priceOverride?.overriddenPrice ?? null,  // FIXED: camelCase field
          effectivePrice: priceOverride?.overriddenPrice ?? template.basePrice,
