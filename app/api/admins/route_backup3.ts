...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!email || !role || !companyName || !monthlyCost || !creditsLimit || !overageCost || !billingCycleDay) {
      return NextResponse.json(
    ...
    const newAdmin = await prisma.$transaction(async (tx) => {
      ...
      // Create subscription
      const subscription = await tx.admin_subscriptions.create({
        data: {
          admin_id: admin.adminId,
          monthly_cost: parseFloat(monthlyCost),
          credits_limit: parseInt(creditsLimit),
          credits_used: 0,
          overage_cost_per_100: parseFloat(overageCost),
          billing_cycle_day: parseInt(billingCycleDay),
          status: "active",
          feature_restaurant: featureRestaurant ?? true,
          feature_reports: featureReports ?? true,
          feature_audio_download: featureAudioDownload ?? true,
          feature_transcript_download: featureTranscriptDownload ?? true,
          feature_bulk_sms: featureBulkSms ?? true,
          feature_sms_confirmation: featureSmsConfirmation ?? true,
        },
      });
