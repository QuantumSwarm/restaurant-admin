...
-- Merged with March 15, 2026 modifications (transcriptions, billing, credit tracking)
-- ===================================================================
...
DROP TABLE IF EXISTS overage_charges;
...
-- ===================================================================
-- 1. Create admin table (updated with company billing fields)
-- ===================================================================
CREATE TABLE admin (
...
COMMENT ON COLUMN admin.stripe_customer_id IS 'Stripe Customer ID for billing integration';
COMMENT ON COLUMN admin.company_name IS 'Company name for invoicing and display';
COMMENT ON COLUMN admin.company_address IS 'Company address for invoicing';
COMMENT ON COLUMN admin.company_phone IS 'Company phone number';
COMMENT ON COLUMN admin.company_fax IS 'Company fax number';
COMMENT ON COLUMN admin.company_url IS 'Company website URL';

CREATE INDEX  idx_admin_stripe_customer ON admin(stripe_customer_id);
...
-- ===================================================================
-- 4. Create store table (updated with sync tracking fields)
-- ===================================================================
CREATE TABLE store (
    ...
    time_zone VARCHAR(63) DEFAULT 'America/Toronto',
    is_active boolean DEFAULT true,
    last_ping TIMESTAMP,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
...
CREATE INDEX  idx_store_last_sync_at ON store(last_sync_at DESC);
CREATE INDEX  idx_store_sync_status ON store(sync_status);
...
COMMENT ON COLUMN store.last_sync_at IS 'Timestamp of the last successful incremental sync from ElevenLabs';
COMMENT ON COLUMN store.last_sync_id IS 'Optional ID or cursor of the last synced item (for resume/recovery)';
COMMENT ON COLUMN store.sync_status IS 'Sync state: idle, syncing, error';
COMMENT ON COLUMN store.sync_error IS 'Last sync error message (if any)';
COMMENT ON COLUMN store.synced_conversations IS 'Count of conversations synced in the last run';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_store_day_unique ON store_hours(store_id, day_of_week);
...
CREATE TABLE customer (
    ...
    phone_number_provided VARCHAR(20),
    name VARCHAR(100),
    session_token UUID DEFAULT gen_random_uuid(),
    email VARCHAR(255),
    preferred_contact_method VARCHAR(10) DEFAULT 'call' CHECK (preferred_contact_method IN ('sms', 'call', 'email')),
    is_active boolean DEFAULT true,
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW()
);
...
-- ===================================================================
-- 8. Create admin_subscriptions table (NEW - Admin-level billing)
-- ===================================================================
CREATE TABLE admin_subscriptions (
    subscription_id SERIAL PRIMARY KEY,
    admin_id INTEGER NOT NULL UNIQUE REFERENCES admin(admin_id) ON DELETE CASCADE,
    ...
    last_overage_check TIMESTAMP,
    billing_cycle_day INTEGER NOT NULL CHECK (billing_cycle_day >= 1 AND billing_cycle_day <= 28),
    current_period_start TIMESTAMP,
    current_period_end TIMESTAMP,
    next_billing_date TIMESTAMP,
    status VARCHAR(50) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'trialing', 'past_due', 'canceled', 'incomplete', 'incomplete_expired')),
...
CREATE INDEX  idx_admin_subscriptions_admin_id ON admin_subscriptions(admin_id);
CREATE INDEX  idx_admin_subscriptions_billing_date ON admin_subscriptions(next_billing_date);
CREATE INDEX  idx_admin_subscriptions_status ON admin_subscriptions(status);
CREATE INDEX  idx_admin_subscriptions_stripe_subscription ON admin_subscriptions(stripe_subscription_id);
COMMENT ON TABLE admin_subscriptions IS 'Admin-level subscriptions with Stripe integration. Each admin has ONE subscription covering all their stores.';
COMMENT ON COLUMN admin_subscriptions.credits_limit IS 'Monthly credit allowance before overage charges apply';
COMMENT ON COLUMN admin_subscriptions.overage_cost_per_100 IS 'Cost per 100 credits over the limit (e.g., $5.00 = $5 per 100 credits)';
COMMENT ON COLUMN admin_subscriptions.credits_used IS 'Credits consumed in current billing period';
COMMENT ON COLUMN admin_subscriptions.billing_cycle_day IS 'Day of month when billing occurs (1-28). Subscription renews on this day each month.';
COMMENT ON COLUMN admin_subscriptions.feature_restaurant IS 'Enable restaurant management features (stores, menus, etc.)';

-- ===================================================================
-- 9. Create subscription table (Store-level - legacy/alternative)
-- ===================================================================
...
-- ===================================================================
-- 10. Create transcriptions table (NEW - Conversation logging)
-- ===================================================================
...
CREATE INDEX  idx_transcriptions_customer_name ON transcriptions(customer_name);
CREATE INDEX  idx_transcriptions_phone_number ON transcriptions(phone_number);
CREATE INDEX  idx_transcriptions_store_created ON transcriptions(store_id, created_at DESC);
CREATE INDEX  idx_transcriptions_text_search ON transcriptions USING gin(to_tsvector('english', transcript_text));
COMMENT ON TABLE transcriptions IS 'Stores conversation transcriptions from ElevenLabs API';
COMMENT ON COLUMN transcriptions.conversation_id IS 'Unique conversation ID from ElevenLabs';
COMMENT ON COLUMN transcriptions.transcript_text IS 'Full text of the conversation transcription';
COMMENT ON COLUMN transcriptions.metadata IS 'Additional data from ElevenLabs API response';
COMMENT ON COLUMN transcriptions.customer_name IS 'Customer name provided during conversation';
COMMENT ON COLUMN transcriptions.phone_number_id IS 'System-captured caller ID from phone system';
COMMENT ON COLUMN transcriptions.phone_number IS 'Phone number verbally provided by customer to agent';

-- ===================================================================
-- 11. Create overage_charges table (NEW - Billing overage tracking)
-- ===================================================================
CREATE TABLE overage_charges (
    charge_id SERIAL PRIMARY KEY,
    subscription_id INTEGER NOT NULL REFERENCES admin_subscriptions(subscription_id) ON DELETE CASCADE,
    ...
    status VARCHAR(50) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'charged', 'failed', 'refunded')),
    charged_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
    CONSTRAINT overage_charges_amount_check CHECK (credits_over_limit > 0 AND amount_charged >= 0)
);
CREATE INDEX  idx_overage_charges_charged_at ON overage_charges(charged_at DESC);
...
COMMENT ON TABLE overage_charges IS 'Records overage charges when credits exceed monthly limit';

-- ===================================================================
-- 12. Create credit_usage_log table (NEW - Credit audit trail)
-- ===================================================================
CREATE TABLE credit_usage_log (
    ...
    source VARCHAR(100) NOT NULL CHECK (source IN ('elevenlabs_call', 'vapi_call', 'manual_adjustment', 'monthly_reset', 'refund', 'other')),
    source_id VARCHAR(255),
    source_metadata JSONB,
    logged_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
    logged_by VARCHAR(100)
);
CREATE INDEX  idx_credit_usage_log_logged_at ON credit_usage_log(logged_at DESC);
CREATE INDEX  idx_credit_usage_log_source ON credit_usage_log(source);
CREATE INDEX  idx_credit_usage_log_subscription ON credit_usage_log(subscription_id);
COMMENT ON TABLE credit_usage_log IS 'Audit trail of all credit usage and adjustments';
COMMENT ON COLUMN credit_usage_log.credits_delta IS 'Credit change amount. Positive = usage, Negative = refund/adjustment';
COMMENT ON COLUMN credit_usage_log.source IS 'Source of credit usage: elevenlabs_call, vapi_call, manual_adjustment, monthly_reset';
COMMENT ON COLUMN credit_usage_log.source_id IS 'External reference ID (e.g., conversation_id from ElevenLabs)';

-- ===================================================================
-- 13. Create prompt table
-- ===================================================================
CREATE TABLE prompt (
...
-- ===================================================================
-- 15. Create menu_template table (Master Menu - Shared Across Stores)
-- ===================================================================
...
-- ===================================================================
-- 16. Create menu_sub_item table (Sub Menu Items)
-- ===================================================================
...
-- ===================================================================
-- 17. Create menu_item_assignment table (Which Stores Offer Which Items)
-- ===================================================================
...
-- ===================================================================
-- 18. Create menu_price_override table (Optional Store-Specific Pricing)
-- ===================================================================
...
CREATE TABLE orders (
    ...
    system_caller_id VARCHAR(36),
    printer_status VARCHAR(10) NOT NULL DEFAULT 'pending' CHECK (printer_status IN ('pending', 'sent', 'error')),
...
-- ===================================================================
-- 22. Create order_capture_logs table
-- ===================================================================
...
-- ===================================================================
-- 24. Create trigger function for automatic updated_at
-- ===================================================================
...
-- ===================================================================
-- 25. Create triggers for tables with updated_at
-- ===================================================================
...
DROP TRIGGER IF EXISTS set_updated_at_restaurant ON restaurant;
CREATE TRIGGER set_updated_at_restaurant
    BEFORE UPDATE ON restaurant
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_store ON store;
CREATE TRIGGER set_updated_at_store
    BEFORE UPDATE ON store
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_customer ON customer;
...
DROP TRIGGER IF EXISTS set_updated_at_subscription ON subscription;
CREATE TRIGGER set_updated_at_subscription
    BEFORE UPDATE ON subscription
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_prompt ON prompt;
CREATE TRIGGER set_updated_at_prompt
    BEFORE UPDATE ON prompt
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_menu_template ON menu_template;
CREATE TRIGGER set_updated_at_menu_template
    BEFORE UPDATE ON menu_template
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_menu_item_assignment ON menu_item_assignment;
CREATE TRIGGER set_updated_at_menu_item_assignment
    BEFORE UPDATE ON menu_item_assignment
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS set_updated_at_menu_price_override ON menu_price_override;
...
DROP TRIGGER IF EXISTS set_updated_at_transcriptions ON transcriptions;
